...
export default class OrderController {
  ...
  public async getOrdersByUserId(req: Request, res: Response): Promise<void> {
  ...
  public async getAddressByUserId(req: Request, res: Response): Promise<void> {
    const { userId } = req.body;
    try {
      const order = await OrderService.getAddressByUserId(userId);
      if (!order) {
        res.status(404).json({ message: 'Order not found' });
        return;
      }
